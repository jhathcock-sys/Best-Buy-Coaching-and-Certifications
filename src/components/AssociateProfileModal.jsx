...
import { generateMonthlyOneOnOne } from '../services/ai';
...
export default function AssociateProfileModal({
  ...
  const renderMarkdown = (text) => {
    return text.split('\n').map((line, i) => {
      ...
      if (line.trim().startsWith('* ') || line.trim().startsWith('- ')) return <li key={i} style={{ marginLeft: '1.5rem', marginBottom: '0.25rem' }} dangerouslySetInnerHTML={{ __html: line.replace(/^[-*]\s/, '').replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>') }} />;
      if (line.trim() === '') return <br key={i} />;
      
      // Handle bold
      const boldedLine = line.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>');
      return <p key={i} style={{ marginBottom: '0.5rem', lineHeight: '1.6' }} dangerouslySetInnerHTML={{ __html: boldedLine }} />;
    });
  };
